...
namespace JQ.Infrastructure
{
    ...
    public sealed class ConnectionMultiplexerFactory
    {
        ...
        {
            if (!_connectionMultiplexerCache.ContainsKey(connection) || _connectionMultiplexerCache[connection] == null)
            {
        ...
        {
            string message = $"redis内部发生错误,{e.EndPoint}:{e.Message}";
            LogUtil.Debug(message, type: typeof(ConnectionMultiplexerFactory));
        }
        ...
        {
            string message = $"redis重新连接时发生错误,{e.EndPoint}{e.Exception.ToErrMsg()}";
            LogUtil.Error(message, type: typeof(ConnectionMultiplexerFactory));
        }
        ...
        {
            string message = $"redis连接时发生错误,{e.EndPoint}{e.Exception.ToErrMsg()}";
            LogUtil.Error(message, type: typeof(ConnectionMultiplexerFactory));
        }
